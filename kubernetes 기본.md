...
데이터를 잃지 않기 위해서는 [퍼시스턴트 볼륨(Persistent Volume)](#https://kubernetes.io/ko/docs/concepts/storage/persistent-volumes/)을 사용하여 전원이 꺼져도 데이터가 유지되는 스토리지 시스템에 데이터를 저장해야 한다.
